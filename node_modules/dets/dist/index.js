...
class DeclVisitor {
    ...
    getNormalProp(node) {
        ...
        if (typeof comment === 'string') {
            return {
                kind: 'prop',
                name: (0, helpers_1.getPropName)(node.name),
                modifiers: (0, helpers_1.getModifiers)(node.symbol),
                optional: node.questionToken !== undefined,
                comment,
        ...
        this.logVerbose(`The prop "${(0, helpers_1.getPropName)(node.name)}" was skipped due to @ignore.`);
        return undefined;
    }
    getIndexProp(node) {
        return {
    ...
    getSetAccessor(node) {
        ...
        this.logVerbose(`The setter "${(0, helpers_1.getPropName)(node.name)}" was skipped due to @ignore.`);
        return undefined;
    }
    getGetAccessor(node) {
        const { checker, flags } = this.context;
        ...
        this.logVerbose(`The getter "${(0, helpers_1.getPropName)(node.name)}" was skipped due to @ignore.`);
        return undefined;
    }
    ...
    getInterface(node) {
        ...
        decls.forEach((m) => {
            ...
            (_b = m.members) === null || _b === void 0 ? void 0 : _b.forEach((p) => {
                props.includes(p) || (0, includes_1.includeProp)(props, p, docs.tags);
            });
...
function getPropName(name) {
    ...
    else if ((0, typescript_1.isNumericLiteral)(name)) {
        return name.text;
...
function stringifyProp(type) {
    ...
    const name = (0, helpers_1.makeIdentifier)(type.name);
...
function includeProp(props, newProp, tags = []) {
    const name = (0, helpers_1.getPropName)(newProp.name);
    // check if we should remove the prop
    if (tags.some((m) => m.name === 'dets_removeprop' && isText(m.text, name))) {
        return;
    }
    for (const oldProp of props) {
        if (oldProp.kind === newProp.kind && (0, helpers_1.getPropName)(oldProp.name) === name) {
